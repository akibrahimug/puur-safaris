...
const iconMap: Record<string, LucideIcon> = {
  Heart, Shield, Users, Globe, Phone, Mail, MapPin, MessageCircle,
}
...
export default async function OverOnsPage() {
  ...
  const aboutSchema = {
    ...
    name: `Over ${stegaClean(siteSettings.siteName)}`,
  }

  const team = aboutPage?.teamMembers ?? []
  const uniquePoints = aboutPage?.uniquePoints ?? []
  ...
  return (
    <>
      ...
      />
      <PageHero
        title={aboutPage?.heroTitle ?? 'Over Puur Safaris'}
        subtitle={aboutPage?.heroSubtitle ?? 'Wij zijn een gepassioneerd team van safari experts.'}
            ...
            <div className="lg:col-span-8 space-y-16">
              ...
              <div className="prose prose-stone max-w-none">
                <h2 className="font-serif text-3xl font-bold text-[var(--text-primary)] mb-6">
                  {aboutPage?.backgroundTitle ?? 'Onze Achtergrond'}
                </h2>
              ...
              <div className="prose prose-stone max-w-none">
                <h2 className="font-serif text-3xl font-bold text-[var(--text-primary)] mb-6">
                  {aboutPage?.missionTitle ?? 'Onze Missie'}
                </h2>
              ...
              <div>
                <h2 className="font-serif text-3xl font-bold text-[var(--text-primary)] mb-8">
                  {aboutPage?.uniquePointsTitle ?? 'Wat maakt ons uniek?'}
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {uniquePoints.map((point) => {
                    const Icon = (point.iconName ? iconMap[point.iconName] : undefined) ?? Shield
                    return (
                    ...
                    return (
                      <div key={member.name} className="flex flex-col gap-4 group">
                        <div className="relative aspect-[4/5] w-full overflow-hidden rounded-3xl border border-[var(--border-subtle)] bg-[var(--bg-secondary)]">
                          {imageUrl && (
                            <Image src={imageUrl} alt={member.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
                          )}
                        </div>
                        <div>
                          <h3 className="font-bold text-lg text-[var(--text-primary)]">{member.name}</h3>
                          {member.role && <p className="text-sm font-semibold text-gold mb-2">{member.role}</p>}
              ...
              <div className="p-8 sm:p-12 rounded-3xl border border-gold/20 bg-gold/5 relative overflow-hidden group">
                ...
                <h2 className="relative font-serif text-3xl font-bold text-[var(--text-primary)] mb-4">
                  {aboutPage?.communityTitle ?? 'Teruggeven aan de Gemeenschap'}
                </h2>
                {aboutPage?.communityText ? (
                  <div className="relative text-[var(--text-muted)] text-lg leading-relaxed mb-8 max-w-2xl">
                    <PortableTextRenderer value={aboutPage.communityText as unknown[]} />
                  </div>
                ) : (
                  ...
                  {siteSettings.phone && (
                    <div className="flex gap-4">
                      ...
                      <div>
                        ...
                        <a href={`tel:${stegaClean(siteSettings.phone)}`} className="text-sm text-[var(--text-muted)] hover:text-gold transition-colors">{siteSettings.phone}</a>
                      </div>
                    </div>
                  )}
                  {siteSettings.contactEmail && (
                    <div className="flex gap-4">
                      ...
                      <div>
                        ...
                        <a href={`mailto:${stegaClean(siteSettings.contactEmail)}`} className="text-sm text-[var(--text-muted)] hover:text-gold transition-colors break-all">{siteSettings.contactEmail}</a>
                      </div>
                    </div>
                  )}
                ...
                <div className="mt-8 pt-8 border-t border-[var(--border-subtle)] flex flex-col gap-4">
                  ...
                  {siteSettings.socialMedia?.whatsapp && (
                    <Button asChild variant="outline" size="lg" className="w-full gap-2 border-[var(--border-subtle)] text-[var(--text-primary)] hover:bg-[var(--border-subtle)] rounded-full">
                      <a href={`https://wa.me/${stegaClean(siteSettings.socialMedia.whatsapp)?.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer">
