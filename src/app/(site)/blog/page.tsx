...
export async function generateMetadata(): Promise<Metadata> {
  const [settings, blogPage] = await Promise.all([getSiteSettings(), getBlogPage()])

  return buildMetadata(
    {
      title: blogPage?.seo?.title ?? 'Blog & Reisverhalen',
      description: blogPage?.seo?.description ?? 'Lees onze nieuwste verhalen, wildlife gidsen en praktische tips voor jouw ultieme safari in Afrika.',
      canonical: '/blog',
    },
    settings
  )
...
export default async function BlogIndexPage() {
  const [blogPosts, blogPage] = await Promise.all([getBlogPosts(), getBlogPage()])
  ...
  return (
    <>
      ...
      <PageHero
        ...
        image={blogPosts[0]?.featuredImage}
      />
        ...
        {stories.length > 0 && (
          <section>
             <div className="flex items-center gap-4 mb-10">
               ...
               <h2 className="font-serif text-3xl font-bold text-[var(--text-primary)]">{blogPage?.storiesSectionHeading ?? 'Verhalen uit de Bush'}</h2>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <article className="relative h-full min-h-[400px] flex flex-col justify-end p-8 sm:p-12 overflow-hidden rounded-3xl border border-[var(--border-subtle)] group-hover:shadow-2xl transition-all duration-700 ease-out group-hover:-translate-y-1">
                      ...
                      <div className="relative z-10 max-w-2xl">
                         <span className="inline-block rounded-full px-4 py-1.5 text-xs font-semibold uppercase tracking-widest bg-gold/90 text-white mb-6 backdrop-blur-md">
                           {blogPage?.featuredBadgeText ?? 'Uitgelicht Verhaal'}
                         </span>
                         ...
                         <div className="flex items-center justify-between text-white/70 text-xs sm:text-sm font-medium">
                           ...
                           <div className="hidden sm:flex items-center gap-2 group-hover:text-gold transition-colors">
                             <span>{blogPage?.readArticleLabel ?? 'Lees Artikel'}</span>
               ...
               <div className="flex flex-col gap-8">
                 {otherStories.map(post => (
                   <BlogCard key={post.slug} post={post} labels={{ readArticleLabel: blogPage?.readArticleLabel }} />
                 ))}
               </div>
             </div>
          </section>
        ...
        {wildlife.length > 0 && (
          <section>
             <div className="mb-10 text-center">
               <span className="text-gold font-bold tracking-widest text-xs uppercase block mb-2">{blogPage?.wildlifeEyebrow ?? 'Flora & Fauna'}</span>
               <h2 className="font-serif text-3xl md:text-4xl font-bold text-[var(--text-primary)]">
                 {blogPage?.wildlifeTitle ?? 'Wildlife Ontdekkingen'}
               </h2>
               <p className="mt-4 text-[var(--text-muted)] max-w-2xl mx-auto">
                 {blogPage?.wildlifeSubtitle ?? 'Van de massale migraties op de eindeloze savannes tot het spotten van zeldzame roofdieren in de struiken.'}
               </p>
             </div>
             ...
             <div className="relative z-10 flex flex-col lg:flex-row gap-12 lg:gap-16 items-start">
                ...
                <div className="lg:w-1/3 lg:sticky lg:top-32">
                   <h2 className="font-serif text-3xl font-bold text-[var(--text-primary)] mb-4">
                     {blogPage?.guidesSectionTitle ?? 'Puur Praktisch'}
                   </h2>
                   <p className="text-[var(--text-muted)] leading-relaxed mb-8">
                     {blogPage?.guidesDescription ?? 'Voorbereiding is essentieel voor een zorgeloze safari. Bekijk onze praktische gidsen en tips over wat in te pakken, de beste tijden om te reizen en hoe je het meeste uit je reis haalt.'}
                   </p>
                   <Link
                     href={blogPage?.guidesCtaLink ?? '/contact'}
                   ...
                   >
                     {blogPage?.guidesCtaLabel ?? 'Vraag Advies Aan Expert'}
                   </Link>
                </div>
        ...
        <section className="relative p-10 sm:p-14 lg:p-20 border-2 border-gold/30 bg-gradient-to-br from-gold/5 to-transparent rounded-[3rem] text-center overflow-hidden flex flex-col items-center group">
          ...
          <div className="relative z-10 max-w-2xl">
            <span className="inline-block rounded-full px-4 py-1.5 text-xs font-semibold uppercase tracking-widest border border-gold text-gold mb-6 backdrop-blur-sm">
              {blogPage?.readerCtaBadge ?? 'Voor Onze Reizigers'}
            </span>
            <h2 className="font-serif text-3xl sm:text-4xl font-bold text-[var(--text-primary)] mb-6 leading-tight">
              {blogPage?.readerCtaHeading ?? 'Heb jij onlangs een onvergetelijke safari met ons beleefd?'}
            </h2>
            <p className="text-lg text-[var(--text-muted)] mb-10 leading-relaxed">
              {blogPage?.readerCtaBody ?? <>Wij zijn gepassioneerd over het vertellen van echte verhalen. Ontwikkel je eigen blogartikel, deel je favoriete reisfoto&apos;s, en inspireer de volgende generatie reizigers met jouw avontuur.</>}
            </p>
            <Link
              href="/blog/inzenden"
            ...
            >
              {blogPage?.readerCtaButton ?? 'Schrijf Jouw Reisdagboek'}
