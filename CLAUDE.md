...
- **`src/proxy.ts`** is Next.js 16's renamed middleware (exported as `proxy`, not `middleware`). For an unprefixed request it picks a locale by priority **country (geo) → Accept-Language → EN**, then redirects `/foo` → `/{locale}/foo` and sets an `x-locale` response header consumed by the root layout. **Geo is authoritative**: anyone physically in a Dutch-speaking country (NL, BE, SR, AW, CW, SX, BQ — the `DUTCH_SPEAKING_COUNTRIES` set) gets `/nl` *even if their browser/OS is set to English* (very common in NL/BE), and everyone else gets `/en`. The country comes from Vercel's `x-vercel-ip-country` header (the same value `@vercel/functions`' `geolocation()` reads; `request.geo` was removed in Next 15). When that header is absent — local dev, non-Vercel host — the proxy degrades to ranked Accept-Language (NL only when explicitly preferred), then EN. **No cookie persistence** — the proxy used to stamp a sticky `NEXT_LOCALE` cookie on every locale-prefixed visit, but without a UI language switcher that just latched onto whichever URL the visitor accidentally landed on (an old bookmark, a shared link) and overrode their real preference for a year; the proxy now actively clears any legacy `NEXT_LOCALE` cookie it sees. `defaultLocale` in `src/i18n/config.ts` stays `nl` because it's the editorial source of truth used by data/SEO.
- **Browser auto-translation is disabled** — `<html translate="no">` + `<meta name="google" content="notranslate">` in `src/app/layout.tsx`. We ship first-class authored translations for both locales, so the browser's "Translate this page?" only ever *degraded* the experience (e.g. an English-browser visitor on `/nl` getting a machine translation layered over authored Dutch). `<html lang={lang}>` (from the proxy's `x-locale` header) still declares the source language for accessibility/SEO. Cross-locale switching is offered by `<LanguageMismatchBanner>` instead, so a visitor on the "wrong" locale is never trapped. `suppressHydrationWarning` on `<html>`/`<body>` plus `<SuppressHydrationWarnings />` (dev-only console filter) stay because Sanity stega + visual-editing still perturb the DOM during hydration. (If a future requirement needs machine translation back as a fallback, remove `translate="no"` and the meta — but then expect Dutch on `/nl` to be re-translated for non-Dutch browsers.)
- **`<LanguageMismatchBanner />` (mounted in the site layout) offers our authored translation** when the visitor's preferred language differs from the page locale — now the *only* path to the other locale since browser auto-translate is off. The banner reads `navigator.languages`, compares to the current page locale, and if a `<link rel="alternate" hreflang="...">` matches the visitor's preferred language, shows a one-click switch. Reads alternates from the rendered head (same source SEO uses) so it works on every page without per-page wiring. Dismissal persists in `localStorage` (`puur:lang-banner-dismissed`). CTA copy is shown in the *target* language (so a non-Dutch reader can actually read it); the two-locale label set lives as a constant in the component, not the dictionary, since it's static and tiny.
