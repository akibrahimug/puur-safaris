...
- **Studio UI is English-only.** This applies to schema `title:` / `description:` / group labels / dropdown option titles / `prepare()` status labels (`src/sanity/schemas/**`), and to `sanity.config.ts` (structure list titles, singleton page entries, studio name, custom delete action labels). Don't reintroduce Dutch labels — that mixes editor language with content language. The `language: "Taal"` label on the `withLanguage()` wrapper is hidden + readOnly anyway, and the i18n plugin's `supportedLanguages` array keeps native-language names ("Nederlands" / "English") by convention.
