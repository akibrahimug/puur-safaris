...
- **Browser auto-translation stays enabled as a fallback safety net** for visitors who somehow land on the wrong locale (old bookmark, shared link, proxy edge case). `<html lang={lang}>` in `src/app/layout.tsx` is set from the proxy's `x-locale` header so Chrome/Edge/Safari know the source language and can offer "Translate this page?". We deliberately do NOT set `translate="no"` / `<meta name="google" content="notranslate">` — those would suppress the prompt and trap a non-Dutch speaker on `/nl` with no exit. Hydration mismatches caused by Translate mutating the DOM are absorbed by `suppressHydrationWarning` on `<html>`/`<body>` plus `<SuppressHydrationWarnings />` (dev-only console filter); React 19 falls back to client-render of the affected subtree in production. If you need to keep specific copy un-translated (brand name, code), put `translate="no"` on the individual element, not the document.
- **`<LanguageMismatchBanner />` (mounted in the site layout) preempts Chrome's machine translation** by offering our authored translation first. Browsers don't expose a hook to inject custom translations into their built-in translate UI — that's a closed pipeline. Instead the banner reads `navigator.languages`, compares to the current page locale, and if a `<link rel="alternate" hreflang="...">` matches the visitor's preferred language, shows a one-click switch. Reads alternates from the rendered head (same source SEO uses) so it works on every page without per-page wiring. Dismissal persists in `localStorage` (`puur:lang-banner-dismissed`). CTA copy is shown in the *target* language (so a non-Dutch reader can actually read it); the two-locale label set lives as a constant in the component, not the dictionary, since it's static and tiny.
